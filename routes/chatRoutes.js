...
// @access  Public
router.get('/project/:projectId/sessions', async (req, res) => {
  try {
...
router.post('/sessions', async (req, res) => {
  try {
    ...
    const chatSession = new ChatSession({
      ...
      sessionId,
      title: title || (projectId ? 'Project Chat Session' : 'General Chat Session'),
      messages: []
    });
  ...
  try {
    ...
    if (searchQuery) {
      try {
        ...
        searchResults = await router.performExternalSearch(searchQuery);
      } catch (searchError) {
    ...
    try {
      ...
      if (agent === 'roadmap') {
        ...
        const roadmapParams = router.extractRoadmapParameters(content);
        ...
        const taskParams = router.extractTaskParameters(content);
        if (taskParams.title && taskParams.description) {
          ...
          response = "I need a task title and description to enhance a task. Please provide both in your message.";
        }
      } else {
        // General agent
        response = await agentService.processQuery(
...
router.get('/sessions/:sessionId/export', async (req, res) => {
  try {
    ...
    if (format === 'json') {
      ...
      const textContent = router.convertSessionToText(session);
...
router.performExternalSearch = async function(query) {
  try {
...
router.extractRoadmapParameters = function(content) {
  const params = {
    timeHorizon: 'quarter',
  ...
  } else if (content.toLowerCase().includes('6 month') || content.toLowerCase().includes('half year')) {
...
router.extractTaskParameters = function(content) {
  const params = {};
...
router.convertSessionToText = function(session) {
