...
const feedbackItemSchema = new mongoose.Schema({
  ...
  source: {
    type: String,
    enum: ['app-store', 'email', 'social-media', 'survey', 'support-ticket', 'user-interview', 'manual'],
    default: 'manual'
  },
  category: {
    type: String,
    enum: ['bug-report', 'feature-request', 'improvement', 'complaint', 'praise', 'question', 'other'],
    default: 'other'
  },
  sentiment: {
    type: String,
...
feedbackSchema.methods.calculateSummary = function() {
  ...
  activeFeedback.forEach(item => {
    // Count categories
    const categoryKey = item.category.replace('-', '').charAt(0).toUpperCase() + 
                       item.category.replace('-', '').slice(1).replace(/([A-Z])/g, '$1');
    if (item.category === 'bug-report') categoryBreakdown.bugReports++;
    else if (item.category === 'feature-request') categoryBreakdown.featureRequests++;
