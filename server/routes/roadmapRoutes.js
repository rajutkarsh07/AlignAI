...
router.get('/project/:projectId', async (req, res) => {
  try {
    const { page = 1, limit = 10, type, timeHorizon, isActive = true } = req.query;
...
router.get('/:id', async (req, res) => {
  try {
    const roadmap = await Roadmap.findById(req.params.id)
      ...
      .populate('items.taskId', 'title status priority category estimatedEffort')
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn('AI roadmap generation failed, creating structured fallback:', aiError);
        // Create a comprehensive fallback roadmap
        roadmapData = router.generateFallbackRoadmap({
          name,
          description,
          type,
          timeHorizon,
      ...
      const normalizedItems = (roadmapData.items || []).map(item => {
        ...
        if (item.timeframe && item.timeframe.estimatedDuration) {
          ...
          if (unit === 'month') item.timeframe.estimatedDuration.unit = 'months';
        }

        // Ensure all required fields have valid values
        return {
          ...item,
          category: ['strategic', 'customer-driven', 'maintenance', 'innovation'].includes(item.category) 
            ? item.category : 'strategic',
          priority: ['critical', 'high', 'medium', 'low'].includes(item.priority) 
            ? item.priority : 'medium',
          businessJustification: {
            ...item.businessJustification,
            riskLevel: ['low', 'medium', 'high'].includes(item.businessJustification?.riskLevel) 
              ? item.businessJustification.riskLevel : 'medium'
          },
          status: ['proposed', 'approved', 'in-progress', 'completed', 'cancelled'].includes(item.status) 
      ...
      const roadmap = new Roadmap({
        ...
        allocationStrategy: roadmapData.allocationStrategy || allocationStrategy,
      ...
      res.status(201).json({
        success: true,
        data: savedRoadmap,
        rationale: roadmapData.rationale || 'Roadmap generated successfully with balanced allocation strategy',
...
router.post('/', async (req, res) => {
  try {
    ...
    if (allocationStrategy) {
      const total = allocationStrategy.strategic + allocationStrategy.customerDriven + allocationStrategy.maintenance;
    ...
    const roadmap = new Roadmap({
      ...
      type,
      timeHorizon,
      allocationStrategy: allocationStrategy || { strategic: 60, customerDriven: 30, maintenance: 10 },
      items
    });
...
router.post('/:id/convert-to-tasks', async (req, res) => {
  try {
    ...
    const itemsToConvert = itemIds.length > 0 
    ...
    for (const item of itemsToConvert) {
      ...
      const task = new Task({
        ...
        category: router.mapRoadmapCategoryToTask(item.category),
        ...
        businessValue: {
          customerImpact: router.scoreToLevel(item.businessJustification.customerImpact),
          revenueImpact: router.scoreToLevel(item.businessJustification.revenueImpact),
...
router.get('/:id/timeline', async (req, res) => {
  try {
    const roadmap = await Roadmap.findById(req.params.id)
      .populate('items.taskId', 'title status priority completionPercentage');
    ...
    Object.keys(timeline).forEach(period => {
      timelineWithProgress[period] = {
        ...
        summary: {
          ...
          completedItems: timeline[period].filter(item => item.status === 'completed').length,
          inProgressItems: timeline[period].filter(item => item.status === 'in-progress').length,
          proposedItems: timeline[period].filter(item => item.status === 'proposed').length
        }
      };
    });
...
router.get('/project/:projectId/analytics', async (req, res) => {
  try {
    ...
    const analytics = router.calculateRoadmapAnalytics(roadmaps);
...
router.mapRoadmapCategoryToTask = function(roadmapCategory) {
  const mapping = {
    'strategic': 'feature',
...
router.scoreToLevel = function(score) {
...
router.generateFallbackRoadmap = function({ name, description, type, timeHorizon, allocationStrategy, project }) {
  const quarters = timeHorizon === 'year' ? ['Q1 2024', 'Q2 2024', 'Q3 2024', 'Q4 2024'] : 
                   timeHorizon === 'half-year' ? ['Q1 2024', 'Q2 2024'] : ['Q1 2024'];
  ...
  return {
    name,
    description: description || `Comprehensive ${type} roadmap for ${project.name}`,
    type,
    timeHorizon,
    allocationStrategy,
    items: fallbackItems,
...
router.calculateRoadmapAnalytics = function(roadmaps) {
  const analytics = {
  ...
  roadmaps.forEach(roadmap => {
    ...
    analytics.byTimeHorizon[roadmap.timeHorizon] = (analytics.byTimeHorizon[roadmap.timeHorizon] || 0) + 1;
    
    // Aggregate metrics
    analytics.averageCompletionRate += roadmap.analytics.completionRate;
    analytics.averageRiskScore += roadmap.analytics.riskScore;
    analytics.averageCustomerSatisfactionPotential += roadmap.analytics.customerSatisfactionPotential;
    
    // Allocation trends
    analytics.allocationTrends.strategic += roadmap.allocationStrategy.strategic;
    analytics.allocationTrends.customerDriven += roadmap.allocationStrategy.customerDriven;
    analytics.allocationTrends.maintenance += roadmap.allocationStrategy.maintenance;
  });
