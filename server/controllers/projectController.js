...
// Create project from uploaded document
const createProjectFromUpload = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    let extractedText =
      'Document uploaded successfully. AI text extraction temporarily unavailable.';
    let formattedPlan = extractedText;

    try {
      const fileBuffer = await fs.readFile(req.file.path);
      extractedText = await aiService.extractTextFromDocument(
        fileBuffer,
        req.file.mimetype
      );
      ...
      try {
        ...
        console.warn(
          'AI formatting failed, using extracted text:',
          formatError.message
        );
